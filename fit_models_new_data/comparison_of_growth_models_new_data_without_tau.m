...
%user-provided dataset. The models DO NOT include the delay
%(starting time) parameter tau of the accompanying paper.
%Instead, all organoids are assumed to start growing at the same time.
...
%Here upper and lower bounds are defined on parameters involved in
%the growth modeling.
ub_powerlaw = 100;
ub_gomp = 10;
lb_growthrate = 0;
...
%In the code, the initial size of the organoid is assumed to be known (it
%is not viewed as an estimated parameter). Therefore, the number of free
%datapoints is one less the number of time points.
n_datapoints = size(timepoints,2) - 1;

%% Definition of other parameters and matrices

%Number of parameters in the 9 growth models we consider,
%without the tau (starting time) parameter.
...
for k=1:no_organoids
    ...
    error = Inf;
        for ell=1:iter
            lb = lb_growthrate;
            ub = ub_gomp;
            initial_guess = (ub-lb).*rand;
            [x_temp, error_temp] = fmincon(@(x) loss_function_exponential(x_0(k,1),x(1),time_series,timepoints),initial_guess,[],[],[],[],lb,ub,[],options);
            if error_temp <= error
                x = x_temp;
                error = error_temp;
            end
    ...
    error_2 = Inf;
        for ell=1:iter
            lb = lb_growthrate;
            ub = ub_powerlaw;
            initial_guess = 10^(log10(ub_powerlaw)*rand);
            [x_temp, error_temp] = fmincon(@(x) loss_function_powerlaw(x_0(k,1),x(1),1/2,time_series,timepoints),initial_guess,[],[],[],[],lb,ub,[],options);
    ...
    error_3 = Inf;
        for ell=1:iter
            ...
            ub = ub_powerlaw;
            initial_guess = 10^(log10(ub_powerlaw)*rand);
            [x_temp, error_temp] = fmincon(@(x) loss_function_powerlaw(x_0(k,1),x(1),2/3,time_series,timepoints),initial_guess,[],[],[],[],lb,ub,[],options);
    ...
    error_4 = Inf;
        for ell=1:iter
            ...
            ub = ub_powerlaw;
            initial_guess = 10^(log10(ub_powerlaw)*rand);
            [x_temp, error_temp] = fmincon(@(x) loss_function_powerlaw(x_0(k,1),x(1),3/4,time_series,timepoints),initial_guess,[],[],[],[],lb,ub,[],options);
    ...
    error_5 = Inf;
    for ell=1:iter
        lb = [lb_growthrate,lb_decayrate];
        ub = [ub_gomp,ub_gomp];
        initial_guess = (ub-lb).*rand(1,2);
        [x_temp, error_temp] = fmincon(@(x) loss_function_Gompertz(x_0(k,1),x(1),x(2),time_series,timepoints),initial_guess,[],[],[],[],lb,ub,[],options);
    ...
    for ell=1:iter
        ...
        ub = [ub_gomp,ub_cc];
        initial_guess = (ub-lb).*rand(1,2);
        initial_guess(2) = 10^(log10(ub_cc)*rand);
        [x_temp, error_temp] = fmincon(@(x) loss_function_logistic(x_0(k,1),x(1),x(2),time_series,timepoints),initial_guess,[],[],[],[],lb,ub,[],options);
    ...
    for ell=1:iter
        lb = [lb_growthrate,lb_decayrate];
        ub = [ub_powerlaw,ub_powerlaw];
        initial_guess = (ub-lb).*rand(1,2);
        initial_guess(1) = 10^(log10(ub_powerlaw)*rand);
        A = [];
        b = [];
        [x_temp, error_temp] = fmincon(@(x) loss_function_vonBertalanffy(x_0(k,1),x(1),x(2),1/2,time_series,timepoints),initial_guess,[],[],[],[],lb,ub,[],options);
    ...
    for ell=1:iter
        ...
        A = [];
        b = [];
        [x_temp, error_temp] = fmincon(@(x) loss_function_vonBertalanffy(x_0(k,1),x(1),x(2),2/3,time_series,timepoints),initial_guess,[],[],[],[],lb,ub,[],options);
    ...
    for ell=1:iter
        ...
        A = [];
        b = [];
        [x_temp, error_temp] = fmincon(@(x) loss_function_vonBertalanffy(x_0(k,1),x(1),x(2),3/4,time_series,timepoints),initial_guess,A,b,[],[],lb,ub,[],options);
...
for k=1:no_organoids
    r(k) = abs(params_5(k,2)-lb_decayrate)<lb_decayrate*10^(-2);
end
...
function sum = loss_function_exponential(v_0,b,a,timepoints)
    sum = 0;
    times = timepoints-timepoints(1);
    for u=1:size(times,2)
        sum = sum + (a(u)-v_0*exp(b*times(u)))^2;
    end
end

function sum = loss_function_powerlaw(v_0,b,gamma,a,timepoints)
    sum = 0;
    times = timepoints-timepoints(1);
    for u=1:size(times,2)
        sum = sum + (a(u)-(v_0^(1-gamma)+(1-gamma)*b*times(u))^(1/(1-gamma)))^2;
    end
end

function sum = loss_function_Gompertz(v_0,b,c,a,timepoints)
    sum = 0;
    times = timepoints-timepoints(1);
    for u=1:size(times,2)
        sum = sum + (a(u)-v_0*exp(b/c*(1-exp(-c*times(u)))))^2;
    end
end

function sum = loss_function_logistic(v_0,b,k,a,timepoints)
    sum = 0;
    times = timepoints-timepoints(1);
    for u=1:size(times,2)
        sum = sum + (a(u)-v_0*k/(v_0+(k-v_0)*exp(-b*times(u))))^2;
    end
end

function sum = loss_function_vonBertalanffy(v_0,b,c,gamma,a,timepoints)
    sum = 0;
    times = timepoints-timepoints(1);
    for u=1:size(times,2)
        sum = sum + (a(u)-(b/c+(v_0^(1-gamma)-b/c)*exp(-c*(1-gamma)*times(u)))^(1/(1-gamma)))^2;
    end
end